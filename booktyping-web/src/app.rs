...
impl Component for TermApp {
	...
	fn create(ctx: &Context<Self>) -> Self {
		...
		panic::set_hook(Box::new(move |panic| {
			_ = web_sys::window()
				.unwrap()
				.alert_with_message(format!("Panicked, please reload. Error: {}", panic.to_string()).as_str());
			panic_hook(panic);
		}));

		let cb = ctx.link().callback(|()| TermAppMsg::Resized);
		let func: Function = Closure::<dyn 'static + Fn()>::new(move || cb.emit(())).into_js_value().into();
